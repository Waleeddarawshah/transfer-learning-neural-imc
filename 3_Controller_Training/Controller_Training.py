...
def denorm_y(y_norm):
    ...
    y[..., 0] = y_norm[..., 0] * Ca_std + Ca_mean
    y[..., 1] = y_norm[..., 1] * T_std + T_mean
    return y
...
assert X_train.shape[-1] == 2, "Expected plant input [Tc_norm, EoverR_norm]."
assert Y_train.shape[-1] == 2, "Expected plant output [Ca_norm, T_norm]."
...
if isinstance(plant_ckpt, dict) and "model_state_dict" in plant_ckpt:
    plant_model.load_state_dict(plant_ckpt["model_state_dict"])
else:
    plant_model.load_state_dict(plant_ckpt)
...
def filter_feasible_set_to_observed_branch(feasible_y, observed_y):
    ...
    if np.sum(mask) < MIN_FEASIBLE_POINTS:
        print("  Warning: feasible support filter was too strict; using percentile-only branch support.")
        mask = (
            (feasible_y[:, 0] >= lower[0])
            & (feasible_y[:, 0] <= upper[0])
            & (feasible_y[:, 1] >= lower[1])
...
fig.tight_layout()
fig.savefig(BASE_DIR / "controller_reference_trajectory.png", bbox_inches="tight")
fig.savefig(BASE_DIR / "controller_reference_trajectory.pdf", bbox_inches="tight")
plt.show()
...
for epoch in range(EPOCHS):
    ...
    if val_tracking < best_val_tracking:
...
fig.tight_layout()
fig.savefig(BASE_DIR / "controller_training_loss_evolution.png", bbox_inches="tight")
fig.savefig(BASE_DIR / "controller_training_loss_evolution.pdf", bbox_inches="tight")
plt.show()
...
def evaluate_controller(C_input, Y_ref):
    controller.eval()
    plant_model.eval()
...
Y_pred_test, U_pred_test = evaluate_controller(C_input_test, Y_ref_test)
...
fig.tight_layout()
fig.savefig(BASE_DIR / "controller_representative_test_prediction.png", bbox_inches="tight")
fig.savefig(BASE_DIR / "controller_representative_test_prediction.pdf", bbox_inches="tight")
plt.show()
